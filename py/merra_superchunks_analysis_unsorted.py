...
Superchunk definition: consecutive chunks in XML document order are merged into
one superchunk when offset2 == offset1 + nBytes1. Chunks are NOT sorted by
offset — evaluation follows the order they appear in the .dmrpp file.
"""
...
def count_superchunks(chunks):
    ...
    Chunks are evaluated in the order given (no sorting).
    """
    if not chunks:
        return 0
    superchunks = 1
