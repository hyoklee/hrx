...
// ---------------------------------------------------------------------------
// GeoTIFF VOL connector helpers
// ---------------------------------------------------------------------------

static hid_t s_geotiff_vol_id = H5I_INVALID_HID;

void h5_set_geotiff_vol_id(hid_t vol_id)
{
    s_geotiff_vol_id = vol_id;
}

bool h5_is_geotiff_filename(const string &filename)
{
    size_t dot = filename.rfind('.');
    if (dot == string::npos)
        return false;
    string ext = filename.substr(dot);
    return (ext == ".tif" || ext == ".tiff" || ext == ".TIF" || ext == ".TIFF");
}

hid_t h5_open_file(const string &filename)
{
    if (s_geotiff_vol_id != H5I_INVALID_HID && h5_is_geotiff_filename(filename)) {
        hid_t fapl = H5Pcreate(H5P_FILE_ACCESS);
        if (fapl < 0)
            return H5I_INVALID_HID;
        if (H5Pset_vol(fapl, s_geotiff_vol_id, nullptr) < 0) {
            H5Pclose(fapl);
            return H5I_INVALID_HID;
        }
