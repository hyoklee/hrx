...
// GeoTIFF VOL connector support.
// Set the registered VOL connector ID; pass H5I_INVALID_HID to disable.
void h5_set_geotiff_vol_id(hid_t vol_id);

// Return true if filename has a GeoTIFF extension (.tif/.tiff/.TIF/.TIFF).
bool h5_is_geotiff_filename(const std::string &filename);

// Open an HDF5 or GeoTIFF file.
// Uses the GeoTIFF VOL connector when active and the filename ends in
// .tif / .tiff; otherwise falls back to H5Fopen with H5P_DEFAULT.
hid_t h5_open_file(const std::string &filename);
