...
bool HDF5RequestHandler::_enable_geotiff = false;
string HDF5RequestHandler::_geotiff_vol_plugin_path;
...
#ifdef HAVE_GEOTIFF_VOL
    if (_enable_geotiff) {
        // H5VLunregister_connector frees the ID; do not call H5VLclose afterward.
        hid_t vol_id = H5VLget_connector_id_by_name(GEOTIFF_VOL_CONNECTOR_NAME);
        if (vol_id != H5I_INVALID_HID)
            H5VLunregister_connector(vol_id);
        h5_set_geotiff_vol_id(H5I_INVALID_HID);
    }
#endif
}
...
#ifdef HAVE_GEOTIFF_VOL
    key_value = obtain_beskeys_info("H5.EnableGeoTIFF", has_key);
    if (has_key)
        _enable_geotiff = key_value;
    BESDEBUG(HDF5_NAME, prolog << "H5.EnableGeoTIFF: " << (_enable_geotiff?"true":"false") << endl);

    if (_enable_geotiff) {
        _geotiff_vol_plugin_path = get_beskeys("H5.GeoTIFFVolPluginPath");
        if (!_geotiff_vol_plugin_path.empty()) {
            if (H5PLappend(_geotiff_vol_plugin_path.c_str()) < 0) {
                BESDEBUG(HDF5_NAME, prolog << "Warning: H5PLappend failed for path: "
                         << _geotiff_vol_plugin_path << endl);
            }
        }
        // Only register if not already registered (e.g. by environment)
        htri_t is_reg = H5VLis_connector_registered_by_name(GEOTIFF_VOL_CONNECTOR_NAME);
        hid_t vol_id = H5I_INVALID_HID;
        if (is_reg <= 0) {
            vol_id = H5VLregister_connector_by_name(GEOTIFF_VOL_CONNECTOR_NAME, H5P_DEFAULT);
        } else {
            vol_id = H5VLget_connector_id_by_name(GEOTIFF_VOL_CONNECTOR_NAME);
        }
        if (vol_id == H5I_INVALID_HID) {
            BESDEBUG(HDF5_NAME, prolog << "Warning: failed to register GeoTIFF VOL connector" << endl);
        } else {
            h5_set_geotiff_vol_id(vol_id);
            BESDEBUG(HDF5_NAME, prolog << "GeoTIFF VOL connector registered, id=" << vol_id << endl);
        }
    }
#endif
...
void HDF5RequestHandler::hdf5_build_das_internal_no_mem_cache(const string& filename, DAS *das, hid_t &cf_fileid)
{
    ...
    if (true == das_from_dc) {
        ...
        if (true == _usecf) {//CF option
            ...
            cf_fileid = h5_open_file(filename);
            if (cf_fileid < 0){
            ...
            depth_first(fileid, "/", *das, filename.c_str());
            close_fileid(fileid);
        }
                                            ...
                                            hid_t h5_fd, bool das_from_dc)
{
    ...
    if (true == _usecf) {
        ...
        depth_first(h5_fd, "/", *das, filename.c_str());
        if(h5_file_open == false)
            close_fileid(h5_fd);
    }
