    ...
    if (handle) {
        ...
        if (!curl::is_url_signed_for_s3(handle->d_url->str())) {
            ...
            if (credentials && credentials->is_s3_cred()) {
                // Use libcurl's native AWS SigV4 signing (equivalent to curl --aws-sigv4 and --user).
                const string sigv4_param = "aws:amz:" + credentials->get(AccessCredentials::REGION_KEY) + ":s3";
                res = curl_easy_setopt(handle->d_handle, CURLOPT_AWS_SIGV4, sigv4_param.c_str());
                curl::eval_curl_easy_setopt_result(res, prolog, "CURLOPT_AWS_SIGV4", handle->d_errbuf.data(), __FILE__, __LINE__);
                const string userpwd = credentials->get(AccessCredentials::ID_KEY) + ":" + credentials->get(AccessCredentials::KEY_KEY);
                res = curl_easy_setopt(handle->d_handle, CURLOPT_USERPWD, userpwd.c_str());
                curl::eval_curl_easy_setopt_result(res, prolog, "CURLOPT_USERPWD", handle->d_errbuf.data(), __FILE__, __LINE__);
            }
        }
#if 0
        AccessCredentials *credentials = CredentialsManager::theCM()->get(handle->d_url);
