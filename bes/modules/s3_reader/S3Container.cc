...
string S3Container::access()
{
    if (!d_dmrpp_rresource) {
        ...
        const string real_url_str = get_real_name();

        // If the URL itself ends in '.dmrpp', it IS the dmr++ file — use it
        // directly rather than appending '.dmrpp' (which would produce a
        // non-existent '<name>.dmrpp.dmrpp' sidecar URL).
        // In this case the HDF5 data URL is the same URL without the '.dmrpp' suffix.
        static const string dmrpp_suffix = ".dmrpp";
        const bool url_is_dmrpp = (real_url_str.size() > dmrpp_suffix.size() &&
                                   real_url_str.compare(real_url_str.size() - dmrpp_suffix.size(),
                                                        dmrpp_suffix.size(), dmrpp_suffix) == 0);

        const string dmrpp_url_str       = url_is_dmrpp ? real_url_str : real_url_str + ".dmrpp";
        const string data_access_url_str = url_is_dmrpp
                                           ? real_url_str.substr(0, real_url_str.size() - dmrpp_suffix.size())
                                           : real_url_str;
