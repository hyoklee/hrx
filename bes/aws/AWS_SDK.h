...
namespace bes
{
    ...
    public:
        ...
        void initialize_s3_client(const std::string &region, const std::string &aws_key,
                                   const std::string &aws_secret_key,
                                   const std::string &endpoint_override = "") override
        {
            d_s3_client = get_s3_client(region, aws_key, aws_secret_key, endpoint_override);
        ...
        std::vector<S3ObjectInfo> s3_list_objects(const std::string &bucket,
                                                   const std::string &prefix,
                                                   const std::string &delimiter = "/") override;
    };
}
#endif // AWS_SDK_H
