                   ...
                   unordered_set<string> &eos5_missing_dim_names)
{
    ...
    // For GeoTIFF files (via VOL connector), H5Lget_name_by_idx and H5OGET_INFO_BY_IDX
    // are not implemented. Use H5Literate2 to collect link names and treat all hard links
    // as datasets (GeoTIFF root group only contains image datasets).
    if (h5_is_geotiff_filename(fname)) {
        ...
        if (nelems > 0) {
            ...
            if (iter_ret < 0) {
                string msg = "h5_dmr handler: H5Literate2 failed for group: ";
                msg += gname;
                throw BESInternalError(msg,__FILE__, __LINE__);
            }
        }
        ...
        for (const auto &lp : link_list) {
            ...
            if (linfo2.type == H5L_TYPE_EXTERNAL || linfo2.type == H5L_TYPE_SOFT)
                continue;

            // Hard link in GeoTIFF root group — always a dataset.
            string full_path_name = string(gname) + oname_str;
            BESDEBUG("h5", "=breadth_first() GeoTIFF dataset: " << full_path_name << endl);
            ...
            if (!is_pure_dim) {
                ...
                vector<char> oname_v(oname_str.begin(), oname_str.end());
                oname_v.push_back('\0');
                handle_pure_dimension(par_grp, pid, oname_v, is_eos5, full_path_name);
            }
        }

        // Group attributes (no child groups in GeoTIFF).
...
string get_hardlink_dmr( hid_t h5obj_id, const string & oname) {
    ...
    // Get the object info.
    // Some VOL connectors (e.g. GeoTIFF) do not implement H5Oget_info;
    // there are no cyclic hard links in such files, so return "" gracefully.
    H5O_info_t obj_info;
