...
/// \return void
///////////////////////////////////////////////////////////////////////////////
void depth_first(hid_t pid, const char *gname, DAS & das, const char *fname)
{
    // Although HDF5 comments are rarely used, we still keep this function.
    ...
    // For GeoTIFF files (via VOL connector), H5Lget_name_by_idx and H5Lget_info
    // are not implemented. Use H5Literate2 to collect link names and types instead.
    bool is_geotiff = (fname != nullptr && h5_is_geotiff_filename(fname));

    if (is_geotiff) {
        // Collect link names and link info via H5Literate2.
        using LinkPair = pair<string, H5L_info2_t>;
        vector<LinkPair> link_list;
        link_list.reserve((size_t)nelems);

        if (nelems > 0) {
            hsize_t idx = 0;
            herr_t iter_ret = H5Literate2(pid, H5_INDEX_NAME, H5_ITER_NATIVE, &idx,
                [](hid_t, const char *name, const H5L_info2_t *info, void *op_data) -> herr_t {
                    auto *v = static_cast<vector<LinkPair>*>(op_data);
                    v->emplace_back(name, *info);
                    return 0;
                }, &link_list);
            if (iter_ret < 0) {
                string msg = "h5_das handler: H5Literate2 failed for group: ";
                msg += gname;
                throw BESInternalError(msg,__FILE__, __LINE__);
            }
        }

        // Process collected links: GeoTIFF only has hard-link datasets in the root group.
        for (const auto &lp : link_list) {
            const string &oname_str = lp.first;
            const H5L_info2_t &linfo2 = lp.second;

            if (linfo2.type == H5L_TYPE_EXTERNAL)
                continue;
            if (linfo2.type == H5L_TYPE_SOFT)
                continue;

            // Hard link in GeoTIFF root group — always a dataset.
            string full_path_name = string(gname) + oname_str;
            BESDEBUG("h5", "=depth_first() GeoTIFF dataset: " << full_path_name << endl);

            add_group_structure_info(das, gname, oname_str.c_str(), false);

            hid_t dset = H5Dopen(pid, full_path_name.c_str(), H5P_DEFAULT);
            if (dset < 0) {
                string msg = "unable to open the GeoTIFF dataset: ";
                msg += full_path_name;
                throw BESInternalError(msg,__FILE__, __LINE__);
            }
            // GeoTIFF VOL does not implement H5Oget_info, so pass num_attr=0.
            try {
                read_objects(das, full_path_name, dset, 0);
            }
            catch (...) {
                H5Dclose(dset);
                throw;
    ...
    for (hsize_t i = 0; i < nelems; i++) {
        ...
        case H5O_TYPE_GROUP: {
            ...
            if (oid.empty()) {    // The group has never been visited, go to the next level.
                depth_first(cgroup, full_path_name.c_str(), das, fname);
            }
            else {
...
void find_gloattr(hid_t file, DAS & das)
{
    ...
    try {
        ...
        // Some VOL connectors (e.g. GeoTIFF) do not implement H5Oget_info;
        // treat as zero attributes in that case.
        int num_attrs = 0;
        H5O_info_t obj_info;
...
string get_hardlink(hid_t pgroup, const string & oname)
{
    ...
    // Get the object info.
    // Some VOL connectors (e.g. GeoTIFF) do not implement H5Oget_info.
    // In that case, there are no cyclic hard links, so return "" gracefully.
    H5O_info_t obj_info;
...
#if (H5_VERS_MAJOR == 2 || (H5_VERS_MAJOR == 1 && ((H5_VERS_MINOR == 12) || (H5_VERS_MINOR == 13) || (H5_VERS_MINOR ==14))))
...
void read_comments(DAS & das, const string & varname, hid_t oid)
{
    ...
    // Some VOL connectors (e.g. GeoTIFF) do not support H5Oget_comment;
    // treat a negative return as "no comment" rather than an error.
    int comment_size;
